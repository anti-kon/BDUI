...
export type Action =
  | { type: 'navigate'; params: { to: string; mode?: 'push'|'replace'|'popToRoot'; params?: Record<string,unknown> } }
  | { type: 'back' }
  | { type: 'popToRoot' }
  ...
  | { type: 'callApi'; params: { url: string; method: 'GET'|'POST'|'PATCH'|'DELETE'; body?: unknown; saveTo?: { scope: Scope; path: string } }; rollbackAction?: Action }
  | { type: 'toast'; params: { message: string; level?: 'info'|'success'|'warning'|'error'; durationMs?: number } }
...
export interface NodeBase { type: string; id?: string; modifiers?: Modifiers; onAction?: Action[]; children?: BDUIElement[]; }

export type TextNode = NodeBase & { type: 'Text'; text?: string; };
export type ButtonNode = NodeBase & { type: 'Button'; title: string; disabled?: boolean; loading?: boolean; };
...
export type RouteScreen = { id: string; title?: string; path?: string; cache?: Record<string, unknown>; node: BDUIElement; };

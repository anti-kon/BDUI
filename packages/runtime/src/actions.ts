...
export function createActionRunner(deps: ActionRunnerDeps): ActionRunner {
  ...
  const { contract, state, navigation, flow, toast, modal, http, validators, prefetchScreens } =
    deps;

  async function handleNavigate(action: NavigateAction): Promise<void> {
  ...
  function handleDec(action: UpdateDecAction): void {
    const current = state.get(action.params.target);
    const byRaw = action.params.by;
    const by = byRaw === undefined ? 1 : toNumber(evaluate(byRaw, state.snapshot()));
    state.set(action.params.target, toNumber(current) - by);
  ...
  function handleToggle(action: UpdateToggleAction): void {
    const current = state.get(action.params.target);
    state.set(action.params.target, !current);
    bus.emit('stateDirty', {});
  }

  function handleAppend(action: UpdateAppendAction): void {
    const current = state.get(action.params.target);
  ...
  function handleMerge(action: UpdateMergeAction): void {
    const current = state.get(action.params.target);
    const patch = resolveValue(action.params.value, state) as Record<string, unknown>;
    const base =
  ...
  function handleMapPath(action: UpdateMapPathAction): void {
    const current = state.get(action.params.target);
  ...
  function handleToast(action: ToastAction): void {
  ...
  function handleModalOpen(action: ModalOpenAction): void {
    modal.open(action.params.id);
  }

  function handleModalClose(action: ModalCloseAction): void {
    modal.close(action.params.id);
  }

  async function handlePrefetch(action: PrefetchScreensAction): Promise<void> {
    if (prefetchScreens) await prefetchScreens(action.params.screens);
  }

  async function handleCall(action: CallAction): Promise<void> {
    if (!http) {
    ...
    try {
      ...
      if (response.status >= 400) {
        if (action.rollbackAction) await run(action.rollbackAction);
        bus.emit('error', {
          action,
  ...
  function dataSourceById(sourceId: string): DataSource | undefined {
  ...
  async function executeHttpDataSource(
    ...
    const headers = resolveValue(source.headers, state, params) as
  ...
  async function handleFetch(action: FetchAction): Promise<void> {
    ...
    const params = (resolveValue(action.params.params ?? {}, state) ?? {}) as Record<
      string,
      unknown
    >;
    try {
  ...
  function handleSync(_action: SyncAction): void {
  ...
  async function handleValidate(action: ValidateAction): Promise<void> {
    ...
    try {
      ...
      const current =
        (state.read('local', '__validation') as Record<string, unknown> | undefined) ?? {};
  ...
  async function handleWhen(action: WhenAction): Promise<void> {
    const pass = evaluateGuard(action.params.if, state.snapshot());
    const branch = pass ? action.params.then : action.params.else;
    if (!branch) return;
    for (const sub of branch) await run(sub);
  }

  function handleFlowStart(action: FlowStartAction): void {
    flow.activate(action.params.routeId);
    if (action.params.params) state.setParams(action.params.params);
    navigation.navigate(action.params.routeId, 'push');
    bus.emit('routeDirty', {});
  }

  function handleFlowAdvance(action: FlowAdvanceAction): void {
  ...
  function handleFlowGoTo(action: FlowGoToAction): void {
    const routeId = action.params.routeId ?? navigation.currentRoute;
    state.write('local', `__flow.${routeId}.current`, action.params.stepId);
    bus.emit('stateDirty', {});
  }

  function handleFlowResume(action: FlowResumeAction): void {
    void action;
    bus.emit('routeDirty', {});
  }

  function handleFlowAbort(action: FlowAbortAction): void {
    const routeId = action.params?.routeId ?? navigation.currentRoute;
    flow.deactivate(routeId);
    bus.emit('routeDirty', {});
  }

  function handleFlowComplete(action: FlowCompleteAction): void {
    const routeId = action.params?.routeId ?? navigation.currentRoute;
    flow.deactivate(routeId);
    bus.emit('routeDirty', {});
  }

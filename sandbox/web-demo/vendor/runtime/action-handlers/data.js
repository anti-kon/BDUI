    ...
    try {
        ...
        if (response.status >= 400) {
            if (action.rollbackAction)
                await ctx.run(action.rollbackAction);
            ctx.bus.emit('error', {
                action,
...
function dataSourceById(ctx, sourceId) {
    return ctx.contract?.dataSources?.find((source) => source.id === sourceId);
}
async function executeHttpDataSource(ctx, source, params) {
    if (!ctx.http)
    ...
    const headers = resolveValue(source.headers, ctx.state, params);
...
export async function handleFetch(ctx, action) {
    ...
    const params = (resolveValue(action.params.params ?? {}, ctx.state) ?? {});
    try {
        const result = source.kind === 'static'
            ? resolveValue(source.value, ctx.state, params)
            : await executeHttpDataSource(ctx, source, params);
        ctx.state.set(action.params.saveTo ?? { scope: 'local', path: `dataSources.${source.id}` }, result);
        ctx.bus.emit('stateDirty', {});
    }
    catch (error) {
        ctx.bus.emit('error', { action, error });

...
describe('compile', () => {
    ...
    it('interpolates templates', () => {
        expect(interpolate('hi {{session.user}}!', { session: { user: 'Ann' } })).toBe('hi Ann!');
    });
    ...
    it('respects boolean operators', () => {
        expect(evalExpression('flow.x > 2 && flow.x < 10', { flow: { x: 5 } })).toBe(true);
        expect(evalExpression('flow.x > 2 || false', { flow: { x: 1 } })).toBe(false);
    });
    it('evaluates ternary expressions', () => {
        expect(evalExpression('flow.x > 0 ? "pos" : "neg"', { flow: { x: 1 } })).toBe('pos');
    });

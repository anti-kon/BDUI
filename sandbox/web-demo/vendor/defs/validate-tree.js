...
function pathOf(parent, index, type) {
    return `${parent}/children[${index}]:${type}`;
}
export function validateTree(root, manifestMap = inferManifestMap()) {
    const issues = [];
    visit(root, '$', [], manifestMap, issues);
    return { ok: issues.length === 0, issues };
}
function visit(node, path, ancestors, manifestMap, issues) {
    ...
    if (nesting?.onlyInside && nesting.onlyInside.length > 0) {
        ...
        if (!parent || !nesting.onlyInside.includes(parent.type)) {
            issues.push({
                ...
                message: `"${node.type}" may only appear directly inside ${JSON.stringify(nesting.onlyInside)}, got "${parent?.type ?? 'root'}"`,
            });
        }
    }

...
Работа содержит 4 рисунка, 0 таблиц, 29 листингов, 19 источников, 3 приложения; итоговое число страниц уточняется после верстки в шаблоне кафедры.
...
– `vitest` версии 4 с адаптером `happy-dom` версии 20 используется в качестве единого каркаса для модульных и интеграционных тестов, включая сценарии отрисовки в эмулированном DOM.
...
– `@bdui/dsl/src/__tests__/contract.test.ts` — проверка сборки корневого DSL-контракта, переноса метаданных, навигации, начального состояния и темы в итоговый объект контракта.
...
– `@bdui/renderer-web/src/__tests__/dom-utils.test.ts` — проверка преобразования модификаторов в CSS, включая вычисление state-driven значений для темизации и пользовательских стилей.
...
### 3.5 Непрерывная интеграция
    ...
    Конвейер непрерывной интеграции определён в файле `.github/workflows/ci.yml` и запускается при каждом push или pull request в основные ветки. Конвейер состоит из задачи `verify`, выполняемой в матрице с Node.js 24, отдельных задач `android` и `ios` для проверки нативных прототипов, а также опциональной задачи `release`, активируемой вручную после успешного прохождения проверок.
...
Листинг 3.2 — Основные шаги конвейера непрерывной интеграции
    ...
    Последовательность шагов подчиняется принципу раннего обнаружения ошибок: сначала выполняется полная сборка (включая кодогенерацию манифестов и схемы), затем — статическая проверка типов, затем — линтинг и проверка форматирования, после этого — запуск тестов, coverage, проверки контрактных артефактов, browser smoke, Taskly smoke и production audit зависимостей. Такой порядок позволяет отклонить изменения ещё на стадии сборки, не тратя ресурсы на более поздние проверки. Опциональная задача `release`, активируемая механизмом `changesets/action`, автоматически создаёт PR с изменением версий и журналом изменений или публикует релиз в npm при наличии соответствующих секретов.
...
### 3.6 Выводы по главе
    ...
    В третьей главе второй части работы описана стратегия и инфраструктура тестирования стека BDUI. Применяемая четырёхуровневая модель (модульные, интеграционные, сценарные и инвариантные тесты) покрывает ключевые функциональные возможности каждой подсистемы и проверяет системные свойства стека в целом. Единая конфигурация Vitest с использованием `happy-dom` для веб-рендерера позволяет проводить проверку DOM-поведения без запуска реального браузера и без разрастания конфигурации между пакетами. Перечень тестов по подсистемам включает двадцать один файл, распределённый по одиннадцати пакетам, и покрывает все существенные алгоритмические блоки. Проверяемые инварианты — детерминизм канонизации, безопасность интерпретатора, согласованность схемы и манифестов, совместимость по SemVer, идемпотентность публикации, корректность ETag-кэширования и stale-while-revalidate, атомарность пакетных действий — являются формальным подтверждением выполнения нефункциональных требований части 1. Конвейер непрерывной интеграции автоматизирует сборку, статический анализ, browser smoke, production audit и прогон тестов на каждом изменении, что исключает попадание неработающего кода в основную ветку и обеспечивает непрерывное соответствие кодовой базы ТЗ.
...
– `public/` — выходной каталог с сгенерированными артефактами `contract.json` и `client.js`, а также вручную поддерживаемым `index.html`, подключающим клиентский бандл тегом `<script type="module" src="/client.js">`.
    ...
    Приложение демонстрирует четыре типовых маршрута. Маршрут `dashboard` иллюстрирует атомарный `batch`, в рамках которого единым действием выполняются HTTP-запрос, очистка черновика, инкремент счётчика и вывод уведомления — при ошибке сервера все изменения откатываются. Маршрут `requests` оформлен как многошаговый flow создания заявки с выбором типа, описанием, бюджетом и финальным подтверждением. Маршрут `settings` сохраняет пользовательские настройки в session-скоуп, позволяет переключать светлую и тёмную тему, а также проверяет корректность двустороннего связывания `Input` и `Select`. Определение серверных действий с откатом и реализация соответствующих API приведены в листингах 4.3 и 4.4.

![Главный экран Taskly](assets/taskly-dashboard.png)

Рисунок 4.1 — Главный экран демонстрационного приложения Taskly

![Настройки Taskly в тёмной теме](assets/taskly-settings-dark.png)

Рисунок 4.2 — Настройки Taskly после переключения на тёмную тему
...
![Веб-демонстрация Campus-контракта](assets/web-demo-settings.png)

Рисунок 4.3 — Web-demo с отрисованным маршрутом настроек Campus-контракта
...
– число тестовых файлов — двадцать один, распределённый по одиннадцати пакетам, с 134 автоматизированными проверками ключевых алгоритмов и сценариев;
...
– число шагов конвейера непрерывной интеграции — полный набор этапов сборки, проверки типов, линтинга, форматирования, тестирования, покрытия, проверки контрактов, browser smoke, production audit, Android assemble, iOS Swift typecheck и опциональной задачи релиза;
...
## ЗАКЛЮЧЕНИЕ
    ...
    Выполненная программа верификации включает двадцать один тестовый файл и 134 автоматизированные проверки с покрытием модульного, интеграционного, сценарного и инвариантного уровней, а также конвейер непрерывной интеграции, автоматически выполняющий сборку, статический анализ, проверку форматирования, browser smoke, production audit и прогон тестов на каждом изменении. Совместно эти средства обеспечивают постоянное соответствие кодовой базы формальным требованиям.
...
## СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ
    ...
    3. Node.js 24 Documentation / OpenJS Foundation. — 2026. — URL: https://nodejs.org/docs/latest-v24.x/api/ (дата обращения: 16.06.2026).
    ...
    10. Vitest Documentation / Vitest Team. — Version 4. — 2026. — URL: https://vitest.dev/ (дата обращения: 16.06.2026).
